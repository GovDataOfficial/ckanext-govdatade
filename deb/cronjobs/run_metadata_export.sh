...
EXPORT_FILE=$EXPORT_DIRECTORY/govdata.de-metadata-daily.json.gz
...
if [ $? -eq 0 ]; then
  ...
  /usr/lib/ckan/env/bin/ckanapi dump datasets --all -q -p 4 -c /etc/ckan/default/production.ini | /usr/bin/jq --slurp . | gzip > $EXPORT_FILE
